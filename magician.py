...
class Spell(Weapon):
	...
	# TODO: __init__
	def __init__(self, name, power, mp_cost, min_level) -> None:
...
class Magician(Character):
	...
	@spell.setter
	def spell(self, val):
		if val is not None and val.minlevel> self.level:
			raise ValueError()
		self.__spell = val
	...
	def compute_damage(self, other):
		...
		if self.will_use_spell():
			self.__mp -= self.spell.mp_cost
			return self._compute_magical_damage(other)
		else: 
			return self._compute_physical_damage(other)

	def will_use_spell(self):
		return self.using_magic and self.spell is not None and self.__mp >= self.spell.mp_cost
